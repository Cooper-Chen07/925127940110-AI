...
private:
    ...
    int m_lastDebugFrame = -9999;                // 上次打诊断信息的帧（每 250 帧一行）
    int m_bronzeUpgradeFrame = -1;               // 铜器升级下令的帧（-1=还没开始升级 → 不采金）
    ...
    std::unordered_map<int,double> m_lastDist;   // 农民SN -> 上帧到目标的距离（判断"有没有在靠近"）
